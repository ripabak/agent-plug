"""LangGraph Postgres checkpointer (conversation state persistence)."""
...
def _psycopg_url() -> str:
    """Convert asyncpg DSN to psycopg DSN (langgraph needs psycopg)."""
...
async def init_checkpointer() -> None:
    """Create + persist the shared AsyncPostgresSaver (idempotent)."""
...
def get_checkpointer() -> AsyncPostgresSaver:
    """Return the initialized checkpointer (raise if not initialized)."""
    if _saver is None:
